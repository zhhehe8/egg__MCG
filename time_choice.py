import numpy as np
# import zhplot # 如果需要中文标签且已安装，可以取消注释 (当前未使用)
...
# --- 汇总图中特定日龄标签的Y轴对齐位置 ---
ALIGNED_LABEL_Y_POSITION = 0.8 # Day 12-17 标签的Y轴位置，已更新
...
def plot_and_save_summary_figure(collected_cycles_data, output_dir, 
    ...
    ax_summary.set_title('Averaged Waveforms(day12-20)', fontsize=15) # 移除子图标题中的 (Offsetted by Day Group)
    # 将X轴标签移动到右侧
            ...
            if day_label: 
                ...
                if len(avg_cycle) > 0:
                    ...
                    if 12 <= day_num_for_align_check <= 17:
                        text_y_pos = ALIGNED_LABEL_Y_POSITION # 使用更新后的对齐位置
                    else:
    ...
    if bar_chart_plot_data:
        ...
        bar_colors = [item['color'] for item in bar_chart_plot_data]
        ...
        bars = ax_bar.bar(bar_x_positions, bar_heights, width=bar_width, color=bar_colors, alpha=0.8)
        
        ax_bar.set_xticks(bar_x_positions)
        ax_bar.set_xticklabels(bar_plot_labels, rotation=45, ha="right", fontsize=9)
        ax_bar.set_ylabel('pT', fontsize=12) # Y轴标题修改
        ax_bar.set_title('R-peak of Averaged Cycles', fontsize=16) # 移除 (Day 12-20)
