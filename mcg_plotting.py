...
def plot_dual_channel_averaging(time_axis_ms, median_beat_bx, dtw_beat_bx, median_beat_by, dtw_beat_by, base_filename, output_dir):
    """ 绘制一张2x1图，整合对比Bx和By信号的两种叠加平均方法(中位数和DTW对齐)。"""
    fig, axs = plt.subplots(2, 1, figsize=(12, 12), sharex=True)
    fig.suptitle(f'叠加平均方法对比 (双通道) - {base_filename}', fontsize=18)
    axs[0].plot(time_axis_ms, median_beat_bx, 'darkorange', linestyle='--', lw=2, label='方法三：中位数平均')
    axs[0].plot(time_axis_ms, dtw_beat_bx, 'blue', lw=2, label='方法四：DTW对齐平均')
    axs[0].set_title('Bx 通道'); axs[0].set_ylabel('幅度')
    axs[0].grid(True, linestyle='--'); axs[0].axvline(x=0, color='r', linestyle='-.', alpha=0.8); axs[0].legend()
    axs[1].plot(time_axis_ms, median_beat_by, 'darkorange', linestyle='--', lw=2, label='方法三：中位数平均')
    axs[1].plot(time_axis_ms, dtw_beat_by, 'green', lw=2, label='方法四：DTW对齐平均')
    axs[1].set_title('By 通道'); axs[1].set_xlabel('相对于R峰的时间 (毫秒)'); axs[1].set_ylabel('幅度')
    axs[1].grid(True, linestyle='--'); axs[1].axvline(x=0, color='r', linestyle='-.', alpha=0.8); axs[1].legend()
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    output_path = output_dir / f"{base_filename}_averaging_comparison_dual.png"
    plt.savefig(output_path)
    print(f"双通道叠加平均对比图已保存至: {output_path}")
    plt.show()
